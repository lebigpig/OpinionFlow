...
interface FalshNewsRepository : JpaRepository<FalshNews, Long> {
    ...
    @Query(
        ...
        countQuery = "select count(f) from falsh_news f where (:start is null or f.send >= :start) and (:end is null or f.send <= :end) and (:q is null or :q = '' or MATCH(f.content) AGAINST(:q IN NATURAL LANGUAGE MODE))",
        nativeQuery = true,
    )
    fun pageAllFiltered(
        @Param("start") start: String?,
