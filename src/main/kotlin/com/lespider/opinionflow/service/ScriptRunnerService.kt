...
import com.lespider.opinionflow.web.dto.ScriptRunResponse
import java.io.File
...
class ScriptRunnerService(
    ...
    private fun normalizeScriptPath(raw: String): String {
        ...
        // 常见：properties 按 ISO-8859-1 解码导致 UTF-8 中文路径乱码（è¡å¸...）
        // 这里尝试把“错误解码后的字符串”按 Latin1 取字节，再以 UTF-8 还原
        val recovered = try {
    ...
    fun run(key: String, code: String? = null): ScriptRunResponse {
        ...
        for ((idx, script) in scripts.withIndex()) {
            ...
            // 读取 stdout/stderr（限制最大字符，避免 OOM）
